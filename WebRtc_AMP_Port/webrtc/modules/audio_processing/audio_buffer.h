...
class AudioBuffer {
 public:
  ...
  float* data_f(int channel); // Marshall: interleaved data
  const float* data_const_f(int channel) const;
  float* const* channels_f(); // Marshall: deinterleaved data
  const float* const* channels_const_f() const;
  float* const* split_bands_f(int channel);// Marshall: split frequency data(2 bands: Low & High)
