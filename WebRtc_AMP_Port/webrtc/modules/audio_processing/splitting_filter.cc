...
void SplittingFilter::ThreeBandsAnalysis(const IFChannelBuffer* in_data,
  ...
  for (int i = 0; i < channels_; ++i) {
	...
	//Remove upper 8kHz, added by Marshall 20160921
	//memset(int_buffer_.get(), 0, sizeof(int16_t) * kSamplesPer16kHzChannel);
	//memset(band2->ibuf()->channel(i), 0, sizeof(int16_t) * kSamplesPer16kHzChannel);
	//memset(band3->ibuf()->channel(i), 0, sizeof(int16_t) * kSamplesPer16kHzChannel);
  }
}
